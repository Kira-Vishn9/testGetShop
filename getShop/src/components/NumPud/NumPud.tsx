...
import { ThemeContext, type ThemeContextProps } from '../../App'
...
const NumPad: React.FC<NumPadProps> = ({
  ...
  const moveDown = (array: Array<Array<string | number>>) => {
    if (currentRow === 1 && currentCol === 2) {
      setCurrentRow(3)
      setCurrentCol(1)
      setActiveButton([3, 1])
    } else if (
      (currentCol === 0 && currentRow === 3 && !isDisabled) ||
      (currentCol === 1 && currentRow === 3 && !isDisabled)
    ) {
      setCurrentRow(4)
      setCurrentCol(0)
      setActiveButton([4, 0])
  ...
  const moveRight = (array: Array<Array<string | number>>) => {
  ...
  const onBackspace = () => {
    setInput(prevInput => {
      ...
      if (lastIndex !== -1) {
        ...
        prevInput =
          prevInput.substring(0, lastNumber) +
          '_' +
          prevInput.substring(lastNumber + 1)
      }
      return prevInput
    })
    setIsDisabled(true)
        ...
        if (i === 4 && j === 0) {
          buttonElements.push(
            <Button
              ...
              className={` ${
                activeButton[0] === i && activeButton[1] === j && !isDisabled
                  ? 'active'
                  : ''
              }`}
          ...
          buttonElements.push(
            <div
              key={buttonsMatrix[i][j]}
              className={`keyboard-button closeButton ${
                activeButton[0] === i && activeButton[1] === j ? 'active' : ''
              }`}
              onClick={() => { handleButtonClick(buttonsMatrix[i][j]) }}
            >
              {buttonsMatrix[i][j]}
